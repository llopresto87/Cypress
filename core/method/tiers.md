...
| **T2** | a bounded change **already authorized** by an active spec and plan — a bug fix with a regression test, a covered increment | Spawn the minimal worker set: one test-first worker may own RED→GREEN in a single context (the spec contract pins the behavior; the reviewer audit stays independent). Close-out spawn (`protocol.canonize`) + full delivery. |

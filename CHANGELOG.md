...
`protocols/ingest-library.md` had a three-role "cast" and a seven-step workflow; `skills/library-wiki` restated the flow twice (creating, refreshing) with a different owner for the index row and a different moment for "promote the page"; `skills/research-and-ingest` restated the scout's steps a third time and ended with "notify docs-librarian"; and `agents/10-research-scout.md` duplicated the skill's source ranking, per-source steps, and conflict rule under its own fact keys. Step 4 (compose the page) had no owner; step 6 (validate) said "one of architect, implementer, or tester"; a `security` spawn re-fetched advisories the scout had already retrieved; and the docs-librarian's mid-pass registration contradicted `canonize`'s single close-out spawn. The exit said "populated" with no definition and no gate. `protocols/from-scratch.md` re-listed grill's creation pass as phases 3–5 in a different order, left phase 6 (the verification baseline) with no owner, and went from test-first straight to deliver — no close-out, against the fail-closed doctrine every other T3 path carries.

### Changed — `protocols/ingest-library.md` is a phase table with a refresh mode and a gated exit
- **`ingest-library.flow`**: identify and corpus-check (the caller, in-session) → `research-scout` retrieves, snapshots, normalizes, registers sources, inspects the code and **drafts** the page (one spawn per dependency, parallel across dependencies) → `tester` smoke-tests the pin → the session writes grill.md §5 → `docs-librarian` finalizes the page and writes the `libraries/index.md` and `sources/index.md` rows **in the close-out spawn**, one spawn for every page the task drafted. Two callers hold the pass — the orchestrator inside grill phase 3 and the librarian during a close-out or docs audit — and in both the scout drafts and the librarian finalizes (`delegation.model-classes`). Advisories the scout finds land in §7 and are weighed by `security` at grill §11; the separate security spawn is gone. A failed smoke test returns to phase 2 as an attempt under `recover`'s three-attempt boundary.
- **`ingest-library.refresh`** is the same pass over an existing page — pin drift, a major, an advisory, a stale "Last reviewed" — with what each phase updates; `research-and-ingest`'s cheap drift check decides *whether* a refresh is due and does not perform one. Exit conditions name the filled §0 pin, the dated review, the sources rows, the tester's smoke test, the grill §5 row, the index row, and `graph-lint.py` green.
- `skills/library-wiki` keeps the discipline (§1–§5) and says what each section of the scout's draft owes; its two workflows become a pointer. `skills/research-and-ingest`'s last step is a handback naming `tester`, not a notification. `agents/10-research-scout.md` drops `research-scout.source-discipline` and `research-scout.conflict-resolution` — the ranking, the per-source steps, and the disagreement rule have one home, `skill.research-and-ingest`, which the scout now `requires:` and applies.

### Changed — `protocols/from-scratch.md`: nine phases with owners, the grill pass reused, the close-out restored
- A phase table — what each phase does, which sub-protocol it adopts, who owns it, what it needs — and the sentence that the table is the spawn order. Phases 3–5 are `grill.flow`'s phases 0–4 and are no longer restated; what remains is the greenfield delta: §1 filled `none — greenfield` on every line (what `grill-lint.py` accepts in place of a path), the shaped options and risks from the brainstorm, and where shaped options die on real research. Phase 6 has owners: `tester` picks the framework (an ADR) and writes the hello-world test, `reliability` writes both runbooks behind verify's one gate entry point, `research-scout` wikifies each tool. Phase 9 is **close-out and deliver**: the one `docs-librarian` spawn that finalizes every page the scouts drafted and writes the index rows, then `deliver`. The exit conditions name both lints green and the close-out's handback.

### Changed — `templates/knowledge-graph/graph-lint.py` checks the ingested page
- `check_libraries` is generalized: besides resolving every `libraries:` edge, it judges every **template-shaped** page under `docs/graph/libraries/` (one with a `## 0. Pin` heading) against the ingest pass's exit — a filled pin table (an exact version, never a placeholder or "latest"), a dated `Last reviewed`, and a row in `libraries/index.md` when the index exists. A page with no pin heading owes nothing here: the growth audit judges a bare page, this check judges an ingested one, and every fixture the other tests build keeps passing. `tests/test_graph_lint.py` gains the class, with a planted violation per rule. `manifest.json`, `CLAUDE.md`, and the reference documentation carry the new homes; `documentation/agents-reference.md` also catches up on 7.6.0's `devils-advocate` allowlist entry and 7.7.0's dropped `tester.test-levels`.

## 7.7.0 — the T3 cycle reads its spawn order from tables: specify and test-first get phase tables, the spec turns active with its RED, and spec-lint checks shape (2026-09-09)

7.6.0 fixed the grill pass. The same review over the rest of the T3 funnel — brainstorm → specify → grill → test-first → verify → canonize → deliver — found the same defect class at both ends of grill, and one contradiction the grill pass did not have.

`protocols/specify.md` had a "who participates" list and a flat twelve-section list with no order between them, and the one order they implied was wrong: `product` was assigned §3 *and* §9 as a single unit, but §9 maps each acceptance criterion to §4 contract slugs that do not exist until `architect` has written them — a product spawn briefed with both writes a §9 that maps to nothing. §0, §1, §2, §5, §8 and §11 had no owner anywhere. `protocols/test-first.md` named phases and not owners; the owners lived in the tester and implementer charters and an orchestrator routing example, with two different COMMIT orders between them, review placed *after* the commit, and the workers appending to grill.md §15 while `canonize` and `verify` say the plan-of-record is session-owned. The test-level table was owned twice (`tester.test-levels` and `test-first.level-selection`).

The contradiction was the spec's `active` moment, stated three ways. `specify` promoted at sign-off. `spec-author` then told the author to "expect the new spec's contracts to report uncovered until test-first lands the RED tests — that failing gate is the spec working". `verify` said a specification is not promoted to a live status until the assertions covering it exist and pass, and the promotion lands in the same change. Under the first two, `spec-lint.py` is red by design for the whole test-first phase — and `deliver` has no FAIL state, so either the delivery is blocked for the length of the feature or someone silences the gate, which is the green lie the rule exists to catch. `verify` was right and owns the moment (`verify.status-evidence`); the other two were the stale copies. And underneath all of it, `spec-lint.py` read the status from a body `**Status:**` line — which the template fills as "see frontmatter (single home)" — so every template-conformant spec parsed as status `see`, was never live, and never had its coverage checked at all.

### Changed — `protocols/specify.md`: `specify.flow` is a phase table, sign-off is not promotion
- The "who participates" list and the section-by-section workflow collapse into one phase table — sections, owner, what it needs, what it may run beside — and the text says *the table is the spawn order*. `product` writes §3 first; `architect` §4–§8; then `product` §9 against the slugs and `tester` §10 with the testability review, side by side; `security` where the surface is sensitive; the orchestrator opens (§0–§2) and closes (§11, sign-offs, §12, grill links). The identifier is the next free number on disk, and the catalog row is the librarian's at close-out — no mid-pass spawn for an index line. A contract that fails testability returns to `architect` as an attempt under `recover`'s three-attempt boundary, not a free loop.
- **Sign-off keeps the status `draft`.** The ticks land in §0; `active` lands in the change that lands the first RED (test-first COMMIT), `implemented` when every contract is green. A signed draft is what `grill` plans against and `test-first` encodes; `spec-lint.py` counts only live specs, so a spec in authoring is never reported uncovered and a live status is never over an empty assertion set. `skills/spec-author/SKILL.md` and `templates/spec.template.md` (the §11 note, the §12 changelog example) follow; `from-scratch`'s exit names the truthful status (`implemented` — the slice's tests are green by then).
- Exit conditions define "populated", gate a §10 row per contract, §9 criteria that resolve, §11 empty or flagged, and `spec-lint.py` green. Two anti-patterns join: §9 written beside §3, and promoted at sign-off.

### Changed — `protocols/test-first.md`: `test-first.cycle` names owners, review comes before commit, the session writes §15
- A phase table above the four phases: RED `tester` → GREEN→REFACTOR `implementer` → REVIEW `reviewer` → COMMIT the session, each with what it needs and what it hands back; one cycle per §9 row in §9 order, the next RED waiting for this COMMIT unless `Depends on:` says the rows are independent (`delegation.sequencing`). The T2 merge (`tiers.execution-paths`) is cited, not restated.
- COMMIT is re-ordered: **review first** (a Critical or Major returns to `implementer` once; a third red reopens `grill`), then the increment's gate, then the session's §15 entry with the cycle's `spawn_id`s in issue order, then the spec's §10 and — in the same change — its status (`draft` → `active` on the first RED, `implemented` on the last green), then the commit. Workers report in the handback; `agents/04-tester.md` and `agents/02-implementer.md` no longer append to grill.md or "hand the diff to" a peer — a leaf hands back naming the next specialist. Entry conditions name a *signed* spec and a lint-green §9; exit conditions name the clean review, the spec status, and both lints.
- `agents/04-tester.md` drops `tester.test-levels`: the level table has one home, `skill.test-first` (`test-first.level-selection`), which the tester now `requires:` and applies.

### Changed — `templates/knowledge-graph/spec-lint.py` reads frontmatter and checks shape
- **Status comes from frontmatter first**, the schema's single home; the body `**Status:**` line is a fallback and the template's "see frontmatter" is never a status. `tests/test-spec-lint.sh` pins this with a spec whose only status is in frontmatter and whose contract is untested: it now FAILs, where before it passed as not-live.
- A **shape pass over every spec on disk**, whatever its status: unique slugs; a §9 "maps to" that resolves to a declared contract; a §10 test-mapping row per contract on any spec that is signed or live (`pending` is a value — the specify exit condition, checked rather than promised); sign-offs present on anything live (a promotion nobody signed FAILs); no `red`/`pending` row on an `implemented` spec; a spec with no `### Failure:` WARNs. The coverage pass over live specs is unchanged. The test suite's fixtures are shape-conformant and each new rule has a planted-violation case.
- `agents/00-orchestrator.md`'s routing examples point at `specify.flow` and `test-first.cycle`; its spec-first enforcement names the signed draft (enough for the T3 funnel) and `active` (required for T2). `protocols/brainstorm.md` names the one template and its place in `grill.flow`. `CLAUDE.md`, `manifest.json`, `DOCUMENTATION.md` and the reference documentation carry the new homes.

## 7.6.0 — grill is a verb: the pass has one spawn order, §1 and §5 have gates that can fail, and the plan is pressed before it is followed (2026-09-09)

The complaint that opened this release was operational: in real sessions the grill pass spawned its specialists in the wrong order, and the discovery and research steps — §1 and the `research-scout` behind §5 — almost never ran at all. Reading the protocol against its own stated spirit explained both. `protocols/grill.md` names a stance ("pressing on the assumptions until they're solid") and implements a form: thirteen numbered steps that each say "fill §N", with step numbers that collide with section numbers (step 2 fills §2–§4, step 3 fills §1) so that any agent normalising the list to section order silently inverts the one ordering the protocol gave a reason for. The same sequence had a second home in `skills/grill-planner/SKILL.md`, which the orchestrator's brief rule points every plan-authoring worker at — and the two disagreed in four places (§1 before §2–§4, `tester` at §9, `security`+`reliability` at §10, no legal checkpoint). A third copy in `documentation/protocols-reference.md` disagreed with both. So in one pass the orchestrator sequenced by the protocol and the worker it spawned sequenced by the skill.

§1 and §5 were the two steps with no working gate. §1 had no exit condition; §5's was vacuous ("every library named in §5 has a wiki page" — an empty §5 satisfies it); both were self-triggered by a judgment the agent makes about its own need, against a standing pressure not to spawn ("available capability is never justification") and a Turn-0 instruction not to bulk-read. The protocol's authors knew: "Skipping §1" and "Skipping §5" were the first two anti-patterns, answered in prose where every other load-bearing claim in the seed gets a lint. And three of the protocol's four `load_when` triggers were revision situations for which the body had no pass at all — the update flow lived only in the skill — so most sessions loaded a creation checklist that did not fit and improvised past the steps that needed a spawn. The router was not the cause: a §5 task line routes `research-scout` at HIGH. It was never asked.

### Changed — `protocols/grill.md` is the one home of the pass, and the pass is a table
- **`grill.flow` is a phase table** — phase, sections, owner, what it needs, what it may run beside — and the text says what the list never did: *the table is the spawn order*; a phase's spawn is issued only after every handback it needs has returned; exactly one pair (§9/§10 slicing and §11 risks, both needing only §8) is parallel-safe. §2–§4 precede §1 with the reason kept; §1 has an owner (router-bounded reads in-session, a sonnet investigation from `investigation-brief.md` beyond that — which also reconciles it with Turn 0's "do not bulk-read", an orientation rule that had read as a prohibition on discovery); §9 is sliced by `architect` and confirmed by `tester`, who owns §10; §10 records divergences from the verification runbook only, which is what its template already said while the protocol asked for a full mapping.
- **§5 is derived, not judged.** Its required set is every `docs/graph/libraries/` page a §9 `Depends on:` row names plus what a §6 decision rests on; an empty §5 says `no external dependency — <reason>`, a falsifiable claim. The scout is spawned per page, and the exit condition that an empty §5 used to satisfy is gone.
- **`grill.revise` is a first-class pass**, moved in from the skill: §15 entry first (with the `spawn_id`s in issue order), struck §9 rows, dated §6 supersessions, and the rule that a revision adding a dependency, moving a boundary, or breaking an assumption re-enters the creation phase that owns it — phase 3 for a dependency, so the scout is spawned in revision exactly as in creation.
- **`grill.press` is the verb.** After §9 and §11 and before §14: the spec↔plan alignment check (unchanged, now half-mechanical); an assumption per increment recorded where the template already had room for it — a §11 row with a verification or a §12 row with its current assumption — with no `[verify]` surviving in §9 or §13; and on a T3 plan, one bounded `devils-advocate` pass over one-way doors and the top risk. `devils-advocate` was on the roster and in no coordinator's `delegates_to`; it joins the orchestrator's.
- **Increment shape** — `Depends on:` names both earlier increments and library pages, rows are listed in dependency order, and blank dependencies join vague tests / no contract / no rollback as what makes a row not ready. **Exit conditions** define "populated" (content, or an explicit `not applicable — <reason>`), gate §1 per line, §5 against §9, §9 order, the press, the §15 spawn list, and `grill-lint.py` green. The two "skipping" anti-patterns become what the lint cannot catch: a `no research needed` you did not earn.
- `skills/grill-planner/SKILL.md` keeps the authoring principles and the audit, and points at `grill.flow`/`grill.revise` instead of restating them — "a worker sequencing from a copy is how spawns come out of order." Its audit leads with the lint and keeps the judgment the lint cannot make. `templates/grill.template.md` says at §1, §5 and §9 what a filled section owes.

### Added — `templates/knowledge-graph/grill-lint.py`, the §3.3 gate
- Installed at `docs/graph/grill-lint.py` beside `spec-lint.py`, add-if-missing, config-free (graft-audit lists it as scaffold; the engine reconciler notes it carries no knob). Dependency-free; `--list` prints the §9 increment graph, `--warn` reports without failing, `--plan P` lints another file, no plan is SKIP.
- It subtracts the template's own lines before judging a section populated, so the form never counts as the plan; a template label with nothing after it is neither content nor "not applicable". Then: every §1 line cited; every §9 row complete, with a dependency on a later row, on itself, or on a missing row a FAIL — that is the exact shape an orchestrator misreads as independence; §5 naming every library page §9 depends on, and every page the plan names existing; every `SPEC-NNNN/SLUG` a row names declared in a spec on disk and every contract of those specs implemented by some row — the alignment check, mechanised; `[verify]` a FAIL in §9/§13 and a WARN elsewhere; §14 one action.
- `tests/test-grill-lint.sh` plants each violation on a valid plan and asserts the lint names it; wired into `tests/run.sh` after `test-spec-lint.sh`. `protocols/verify.md` gains the gate row; `install.sh`, `tests/test-full-install.sh`, `tools/graft-audit.py`, `tools/graft-graph-engine.py`, `manifest.json` and the documentation carry the file wherever they carried `spec-lint.py`.

### Changed — `core/method/delegation.md` owns sequencing
- `delegation.sequencing`: the independence rule that lived only in the orchestrator's charter — invisible to the five other coordinators and dropped by the role-emulation fallback — moves to the method node with the part it lacked: the sequence is *read*, from a protocol's phase table or grill.md §9, never improvised from a flat list; a spawn is issued after the handbacks it needs; the `spawn_id` ordinals are the record of the order used. The emulation fallback restates it alongside the depth ceiling. `agents/00-orchestrator.md` points at it and gains the `devils-advocate` routing line.

## 7.5.0 — expertise composes through the graph: the staffing decision is recorded and spawnable, its default answer is a node, and the router descends (2026-09-09)

7.3.0 made growth prove what it put on the plant's shelves. It did not ask the other half of the question: growth is also supposed to leave behind the expertise this project needs and the shipped roster does not have. The machinery for that was all present and none of it was load-bearing. The evidence ledger's §9 collected specialist signals, the author brief mapped them to "a project-specific specialist agent", `agent-corpus/` held candidate roles — and then the decision was written to `.cypress/growth/`, which is gitignored and discarded at the end of the run. A plant kept no answer to "did growth consider what expertise this project needs?", so a §9 weighed and declined read exactly like a §9 nobody opened. Worse, an expert that WAS authored never became one: `install.sh` projects `.claude/agents/` and its siblings from the seed's own roster, `graft.md` re-projects the plant's agents at Phase 7, and `grow.md` projected nothing — so an expert authored into `docs/graph/agents/` sat there unspawnable, because the host reads its roster from the projection directory when a session starts. Growth could report a specialist the plant could never call.

Closing that exposed the larger defect underneath it: an agent was the only answer the seed had. Every piece of project-specific expertise had to become a registered roster entry with its own triggers, its own registration, and a fresh context per spawn — and the delegation graph is deliberately capped at depth 3, so an implementer reaching library knowledge through agents would consume the whole budget with no headroom. What the operator actually needed was cheaper and narrower: expertise that composes as **nodes and leaves**, resolved by the router that already runs in every brief.

The graph had no edge for it. `requires:` is a hard dependency always loaded with its node — composing a stack's libraries through it drags every library page into every task that touches the stack, which `_schema.md` already names as an anti-pattern. `peers:` is the opposite and equally wrong. So 7.5.0 adds the missing one: `composes:`, lazy and task-conditioned, the menu a router reads rather than a closure it swallows. A task about the persistence layer loads the runtime expertise and that library's node, and never touches its siblings. The staffing question survives, sharper: the node is now the default answer and is owed mechanically, and an agent is warranted only for what a node cannot be. Decided in `docs/decisions/adr-0005-composable-expertise-as-graph-nodes.md`.

### Added — the `expertise` kind and the lazy `composes:` edge
- `kind: expertise`, at `docs/graph/nodes/expertise.<slug>.md` beside `deviation.*` and under the same `nodes/` roof — one graph, not a second one. It owns exactly two facts: `<slug>.applicability` (when this is in play, and what must not be done without it) and `<slug>.composition` (which sub-expertises apply under which condition). Every API detail, pin, and standard stays in `libraries/` and `best-practices/`, reached by `libraries:`/`artifacts:`; a node with no depth edge routes to nothing and fails lint (rule 18).
- `composes:` — lazy, downward, expertise-to-expertise only (rule 15). Its union with `requires:` is deliberately NOT cycle-checked, because `parent composes child` together with `child requires parent` is the intended shape: the eager edge points up to what you cannot be correct without, the lazy one down to what this task may not need. Each is acyclic on its own (rules 6 and 16), and a child that `requires` an expertise parent must appear in that parent's `composes:` — lint names the line to add (rule 17), which is what makes a downward edge safe to author one file at a time.
- **Delta-exact descent.** The router descends into a composed child when the task names, EXACTLY, a term in the child's own vocabulary — its `load_when` plus its whole slug — that the parent does not carry. Three properties fall out and each was the reason for a rejected alternative. Family words sit on the parent by construction, so they descend nobody. It is IDF-free, so a term's weight cannot shift as the plant grows and change what routes. And descent never prefix-folds, so a task about "migrating the CI runner" cannot COMPOSE schema-migration knowledge off a word that names nothing about it — seeding still folds exactly as it did, and the absence of a composed-by line on a loaded node is what tells the two apart.
- Slugs are unversioned. The pin lives in `libraries/<slug>.md` and the node points at it; putting a version in an id would spread that pin into every referrer's edge list, which rule 10 exists to prevent. The one exception is a plant running two majors of one stack at once, where applicability genuinely differs: the unversioned node then composes one version-qualified child per major, derived from the inventory rather than decided, and legal only in that shape (rule 19).
- The seam with `stack.*` is declared in both bullets of the schema's kinds list: a stack node owns this project's own conventions and requires its expertise node; an expertise node owns applicability over external depth. Neither restates the other, and a graft that finds routing prose in a stack node MOVES it rather than copying it.

### Added — `templates/docs/nodes/_expertise.template.md`
- The form, modelled on `_deviation.template.md`. Its body order is fixed: what this is in play for, what you must not do without it, the composition and the condition each child applies under, **version in play**, and a depth map from purpose to leaf. The version section is a pointer, never a value — it is what stops the node becoming a second home for the pin, and it is what a reader of any identity follows to the right leaf. Placeholders are `{{ }}` rather than the deviation form's `<...>`, because that is what the coverage gate's placeholder and brace-normalizing measures understand; a form whose braces were rubbed off would otherwise read as authored content.

### Added — the expert row set and the staffing decision
- `tools/growth-audit.py` gains a fourth row set, `experts`, derived from the plant's own graph: every agent under `docs/graph/agents/` that the SEED did not put there, plus any an inventory row names. Like the collection and agent sets, it is derived rather than declared, so an expert cannot leave the audit by being left out of the record. Seed-ness is derived from the seed's roster and never from the file's own `origin:` — trusting the declaration inverted the check it was paired with, because an expert copied from a seed agent as a starting point keeps `origin: seed`, and the one wrong value the `origin: project` finding exists to catch was the single value that hid the file from it. Rows are keyed by FILE STEM, which is what the installer projects by; keying by the frontmatter `name` asked for a projection at a path nothing writes and collapsed two files sharing a name into one row.
- Inventory items of kind `domain`, and any item marked `significance: core`, carry an `expert` decision — `warranted` plus `why`, a `name` when it is true, and a `needs` naming which of `tools`, `model`, `stance`, `isolation` that agent needs and its expertise node cannot serve. Leaving the question unasked is the verdict `UNSTAFFED`, along with `warranted: false` carrying no reason, a `name` that resolves to no expert the plant carries, and a `warranted: true` that names no trigger.
- An expert owes four things beyond existing, each of which had failed silently: `origin: project` in its frontmatter, a `plant_knowledge:` list, a `motivated_by` citation resolving to the source that earned it (an agent file is not that source — a roster cannot be its own evidence), and a projection at every path the plant's stamp records. A missing projection is `UNGROWN` and names the adapter that cannot spawn it; a projection edited away from its graph home is `CONTRADICTED` where the adapter copies verbatim, and checked for presence only where the adapter transforms it at install time.
- A form is not an expert, and it is not a node either. `is_substantive()` compared a file against the seed template at the SAME path, and two artifact classes have none — an agent node and an expertise node are authored from forms that live nowhere near them — so the form's own instructional prose counted as this plant's authored content and a filled-in copy passed. `scaffold_for()` is now the one place that answers "what was this authored from", for both, and the authored-bytes measure normalizes placeholder braces, because rubbing the `{{ }}` off every line was enough to make a whole form read as written.

### Added — `install.sh` records where a roster is spawnable from
- `agent_projection_for` is the ONE home of the adapter-to-directory mapping, and `write_seed_stamp` records what it resolved into the plant's `.cypress/seed.json` as `agent_projections` — the path pattern per adapter and whether that projection is byte-identical to its graph home or generated at install time. The audit reads the paths from there. Keeping a copy of the mapping in the tool was a second home for a fact the installer owns, and the lint guarding it could not see a renamed directory or a changed filename pattern. A stamp that records no `agent_projections` while the plant carries experts is `STALE`, not a silent pass.

### Changed — `templates/knowledge-graph/graph-lint.py`
- `KINDS` gains `expertise`; `LIST_KEYS` gains `composes`. `TRAVERSAL_EDGES` and `Node.out_edges()` are now the one home of "the edges a traversal follows" — reachability walked `requires + peers` from two hand-written lists, so a third edge would have been reachable in one walk and not the other. `check_acyclic` takes the edge it checks as an argument and runs twice rather than being copied; `check_edges` validates all three and enforces the expertise-only rule for `composes`.
- New per-kind check `check_expertise` (rules 17–19) and a new warning arm `check_composition_triggers`: the `load_when` analogue of `agent-lint`'s routing-trigger warning, applied to composed children. A term two siblings share is family vocabulary that belongs on the parent, where it cannot descend either of them; a term more than three nodes carry is too generic to say this child is what the task is about. Both ignore tokens under three characters, because `_terms` drops those from every task, so the `0` in a target-framework token can never match and must never be reported.
- `--plan` now says how each node arrived — a seed, a `requires` of something, or composed by a parent on a named term — and replaces the peers-only skip list with ONE `NOT LOADED` section carrying a reason per line, because "not loaded, and here is why" is one idea. It also warns when a parent hands over most of a real menu at once. `--graph` prints `->` for requires and `~>` for composes. A graph carrying no `composes:` routes exactly as it did before, which its own regression case pins over the resolved id sets rather than over the printed text.

### Changed — `tools/growth-audit.py`
- `KIND_PLAN` gains a third column and `planned_artifacts()` is now the single home of everything an inventory item owes: the base artifacts its kind implies, the incidental exception (unchanged — an incidental DEPENDENCY still earns an index line and no grounding, while an incidental item of any other kind keeps its pages and its grounding), the expertise node, and one version-qualified child per major where `majors_in_play()` finds two. The incidental rule had been an `if` sitting beside the table that owned every other rule.
- An expertise node owed by a pinned kind must name that item's own library page in `libraries:`, or it is `HOLLOW`: it owns applicability, never the version, so a node that routes to no pin home has nowhere to put the distinction and becomes the second home it was designed not to be. An expertise path planned for an incidental item is `BLANK` — over-growth is a finding on the same footing as a gap.
- `plant_knowledge:` now accepts a node id alongside a collection and a single row, resolved through one `read_target()`. This is for a plant-authored expert declaring the expertise it draws on; a node the plant does not carry is `DANGLING` rather than reported as an empty collection. No seed agent declares expertise — the roster reaches it through the router.

### Changed — the contract and the intake
- `_schema.md` describes three edges from its opening paragraph rather than two, carries the `expertise` kind beside the `stack` bullet it shares a seam with, and gains rules 15–19. Rules 1–14 keep their numbers, which `graph-lint.py` cites by number.
- `protocols/grow.md`: the shape diagram names what lives under `nodes/`; Phase 4's `libraries/` and `best-practices/` bullets are rewritten to describe the three artifacts one body of retrieved evidence yields, the node routing to the other two and restating neither; "Staff the project" now says the node is owed and an agent needs a trigger; Phase 5's librarian bullet names the new trigger warnings as its mechanical form; Phase 6's validation item on small closures now covers descent.
- `templates/prompts/growth-evidence-ledger.md`: §9 is reworked rather than joined by a new section — it already asked what project-specific expertise a plant needs, and most of that answer is now a node. It gathers what each core or significant stack element is used with, the task shapes it is in play for in the ≥3-character words the router can actually match, what goes wrong without it, and whether two majors are in play.
- `growth-coverage-record.md`, `growth-author-brief.md`, `agents/growth-scout.md`, `agents/growth-orchestrator.md`, `templates/library-page.template.md` (its §0 is now a "versions in play" table rather than a scalar pin above one), `templates/knowledge-graph/index.md`, and the router-facing skills follow the same seam in their own voices.
- `core/method/delegation.md`: routing on a LOW/NONE band now asks first whether the gap is knowledge or judgment that needs its own context. Knowledge is a node the router composes in; only the four triggers commission an agent. The same supersession lands in `agents/00-orchestrator.md`, `protocols/recover.md`, `protocols/harvest.md`, `templates/prompts/investigation-brief.md`, and `templates/agent.template.md`'s header.
- `protocols/graft.md`: Phase 5 grows expertise nodes onto a plant grown under an older seed and MOVES applicability prose out of any `stack.*` node that carries it, leaving a `requires:` edge — a graft may not leave one routing fact in two nodes. Phase 7 names the `composes` rules among what the upgraded engine enforces.

### Changed — `tests/`
- `test_graph_lint.py` gains twenty-one cases across two classes. The contract class covers the edge's own rules; the descent class covers what `--plan` does with it, and **every descent case asserts the provenance line or the not-loaded reason on a task the UNMODIFIED tool did not seed the child on**. That discipline is not cosmetic: an independent review of this plan measured the first draft's fixture and found seeding already loaded every child, so a membership assertion would have passed with the descent code deleted. Each such task's wording is recorded in the test that uses it.
- `test-growth-audit.sh` gains eleven cases (28–38): who owes an expertise node and who does not, the incidental valve in both its forms, a brace-stripped form rejected as a node, a node that routes to no pin home, the two-majors shape, a plant that owes none going green with no row to answer, an expert declaring the node it reads, and a staffing decision that names no trigger.
- `seed-lint.py` holds the contract and its linter to each other in both directions — a kind the linter accepts and the schema never describes is a shape nobody knows how to author, and an edge the schema promises and the linter ignores is a rule that is not one — and generalises its template-key arm to cover both forms a plant authors nodes from.

...
- **Delta-exact descent.** The router descends into a composed child when the task names, EXACTLY, a term in the child's own vocabulary — its `load_when` plus its whole slug — that the parent does not carry. Three properties fall out and each was the reason for a rejected alternative. Family words sit on the parent by construction, so they descend nobody. It is IDF-free, so a term's weight cannot shift as the plant grows and change what routes. And it never prefix-folds, so a task about "migrating the CI runner" cannot pull in schema-migration knowledge, while seeding's fold is left exactly as it was.

...
This creates:
- `CLAUDE.md` → symlink (or copy on Windows) to `core/AGENTS.md`
- `.claude/agents/*.md` → symlinks to `agents/*.md`
- `.claude/skills/<name>/SKILL.md` → symlinks to `skills/<name>/SKILL.md`
...
If `.claude/` already exists with custom content, the installer
prompts before touching it. Conflicts are reported, not silently
overwritten.
